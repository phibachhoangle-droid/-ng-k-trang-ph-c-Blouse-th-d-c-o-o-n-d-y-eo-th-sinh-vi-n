...
function doGet(e){
  ...
  return json_({ok:true,service:'Trang phục sinh viên backend',version:'2026-09-11-timeout-fix'});
}
...
function saveRegistration_(data){
  ...
  const hasHat=qBlouseSet>0||qBlouseHat>0;const hatType=(major==='Điều dưỡng'&&hasHat)?(gender==='Nam'?'Mũ Nam':'Mũ Nữ'):'';const transferContent=`${name} - ${studentId}`;

  const lock=LockService.getScriptLock();lock.waitLock(20000);let row,proofUrl='';
  try{
  ...
  let emailSent=false;
  try{
    sendConfirmation_(data,{name,studentId,cls,major,gender,hatType,transferContent,total,blouseMode,qBlouseSet,qBlouseShirt,qBlousePants,qBlouseHat,qSport,qUnion,qBag,qLanyard,blouseSetMoney,blouseShirtMoney,blousePantsMoney,blouseHatMoney,sportMoney,unionMoney,bagMoney,lanyardMoney});
    emailSent=true;
  }catch(mailErr){
    console.error('Email error',mailErr);
    const c=sh.getRange(row,40),old=clean_(c.getValue());
...
function validatePerson_(d){
  ...
  required.forEach(k=>{if(d[k]===null||d[k]===undefined||clean_(d[k])==='')throw new Error(`Thiếu trường ${k}`)});
  ...
  const major=clean_(d.majorName),cls=clean_(d.className),other=d.classWasOther===true||String(d.classWasOther).toLowerCase()==='true';
...
function sendConfirmation_(data,x){
  const email=clean_(data.email);if(!email)return;
  const lines=[];
  if(x.qBlouseSet)lines.push(`- Bộ blouse (áo + quần + mũ): ${x.qBlouseSet} bộ | Size ${clean_(data.blouseSetSize)}${x.hatType?' | '+x.hatType:''} | ${money_(x.blouseSetMoney)}`);
  ...
  if(x.qBlouseHat)lines.push(`- Mũ blouse: ${x.qBlouseHat} cái${x.hatType?' | '+x.hatType:''} | ${money_(x.blouseHatMoney)}`);
  ...
  const body=[`Chào ${x.name},`,'','Hệ thống đã ghi nhận đăng ký và ảnh minh chứng chuyển khoản của bạn.',`MSSV: ${x.studentId}`,`Ngành / nhóm: ${x.major}`,`Lớp: ${x.cls}`,`Giới tính: ${x.gender}`,`Nội dung chuyển khoản: ${x.transferContent}`,'','CÁC HẠNG MỤC ĐÃ ĐĂNG KÝ:',...lines,'',`TỔNG THANH TOÁN: ${money_(x.total)}`,'','Trạng thái: Chờ kiểm tra chuyển khoản.','Vui lòng giữ email này để đối chiếu khi cần.'].join('\n');
  MailApp.sendEmail(email,subject,body);
}

function qty_(v){const n=Number(v||0);return Number.isFinite(n)&&n>=0&&n<=5?Math.floor(n):0}
function clean_(v){return String(v==null?'':v).trim()}
function normalizeCode_(v){return clean_(v).normalize('NFD').replace(/[\u0300-\u036f]/g,'').replace(/Đ/g,'D').replace(/đ/g,'d').replace(/[^A-Za-z0-9]/g,'').toUpperCase()||'NA'}
